...
const nothingToPreview = 'Rien à prévisualiser';
...
export class NewCommentComponent {
    ...
    this.element.innerHTML = `
      ...
      <form class="comment" accept-charset="UTF-8" action="javascript:">
        ...
        <div class="comment-body">
          <textarea class="form-control" placeholder="Laisser un commentaire" aria-label="comment"></textarea>
        ...
        <footer class="new-comment-footer">
          <a class="text-link markdown-info" tabindex="-1" target="_blank"
            ...
            Il est possible d'ajouter des styles avec Markdown
          </a>
          <button class="btn btn-primary" type="submit">Commenter</button>
          <a class="btn btn-primary" href="${getLoginUrl(page.url)}" target="_top">S'authentifier pour commenter</a>
        </footer>
      </form>`;
  ...
  public setUser(user: User | null) {
    ...
    if (user) {
      ...
      this.textarea.placeholder = 'Laisser un commentaire';
    } else {
      this.avatarAnchor.removeAttribute('href');
      this.avatar.alt = '@anonymous';
      this.avatar.src = anonymousAvatarUrl;
      this.textarea.disabled = true;
      this.textarea.placeholder = 'Il faut s\'authentifier sur Github pour commenter.';
    }
  }
